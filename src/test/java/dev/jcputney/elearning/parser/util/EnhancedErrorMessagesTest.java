...
class EnhancedErrorMessagesTest {
  ...
  void testLocalFileAccessEnhancedErrors(@TempDir java.nio.file.Path tempDir) {
    ...
    assertTrue(exception.getMessage().contains("full path"));
  ...
  void testLocalFileAccessDirectoryNotFound(@TempDir java.nio.file.Path tempDir) {
    ...
    assertTrue(exception.getMessage().contains("nonexistent-directory"));
    assertTrue(exception.getMessage().contains("full path"));
    assertTrue(exception.getMessage().contains("root"));
  }
  ...
  void testZipFileAccessEnhancedErrors() {
    ...
    assertTrue(exception.getMessage().contains("/nonexistent/path/file.zip"));
  }
  ...
  void testInvalidRootDirectoryError(@TempDir java.nio.file.Path tempDir) throws IOException {
    ...
    // Verify the error message contains enhanced context
    assertTrue(exception.getMessage().contains("Invalid root directory"));
    assertTrue(exception.getMessage().contains("path points to a file"));
    assertTrue(exception.getMessage().contains(regularFile.toString()));
  }
}