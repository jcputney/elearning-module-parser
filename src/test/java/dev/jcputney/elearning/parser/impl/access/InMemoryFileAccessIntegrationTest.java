...
/**
 * Integration tests for {@link InMemoryFileAccess} with actual module parsers. Tests loading real
 * SCORM packages from the test resources into memory and parsing them.
 */
  ...
  private byte[] loadTestModule(String modulePath) throws IOException {
    ...
    if (!Files.exists(path)) {
      ...
      if (Files.isDirectory(Paths
          .get(TEST_MODULES_PATH)
          .resolve(modulePath))) {
        return createZipFromDirectory(Paths
            .get(TEST_MODULES_PATH)
            .resolve(modulePath));
      }
