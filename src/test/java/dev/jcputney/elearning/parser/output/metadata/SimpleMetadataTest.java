...
class SimpleMetadataTest {
  ...
  void testMethodChaining_MultipleAdditions_Success() {
    ...
    assertEquals("Test", metadata.getMetadata("title").get());
    assertEquals(1, metadata.getMetadata("version").get());
    assertEquals(true, metadata.getMetadata("active").get());
  }
  ...
  void testEquals_SameContent_ReturnsTrue() {
    ...
    metadata.addMetadata("key1", "value1").addMetadata("key2", 42);
    other.addMetadata("key1", "value1").addMetadata("key2", 42);
