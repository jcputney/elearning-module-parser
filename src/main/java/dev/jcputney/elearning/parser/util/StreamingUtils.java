 ...
 * Provides enhanced streaming capabilities including progress tracking and buffering.
 */
public final class StreamingUtils {
  
  private static final Logger log = LoggingUtils.getLogger(StreamingUtils.class);
  ...
  public static InputStream createProgressTrackingStream(
    ...
    return createProgressTrackingStream(inputStream, totalSize, listener, DEFAULT_PROGRESS_INTERVAL);
  }
  ...
  private static class ProgressTrackingInputStream extends FilterInputStream {
    ...
    public void close() throws IOException {
      try {
        ...
        log.warn("Error notifying progress listener on close", e);
      } finally {
        super.close();
      }
    }
    ...
    private void updateProgress(long bytes) {
      ...
      if (total - lastProgressUpdate >= progressInterval) {
        ...
        try {
          ...
          log.warn("Error notifying progress listener", e);
        }
      }
    }
  }
